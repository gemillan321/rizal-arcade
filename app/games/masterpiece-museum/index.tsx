...
export function MasterpieceMuseumGame({ onClose }: { onClose: () => void }) {
  ...
  // Same reasoning as Hearts & Horizons: the desk advances itself on phones
  // instead of asking the player to manually flip between three tabs.
  const mobileStep: "gallery" | "plaque" | "ready" =
    wrongSelection === "gallery" || wrongSelection === "both" ? "gallery"
    : wrongSelection === "plaque" ? "plaque"
    ...
    <>
      ...
      <section className="museum-game play-layout">
        ...
        <div className="museum-workspace">
          <p className="museum-mobile-step" aria-live="polite">
            {mobileStep === "gallery" ? "Step 1 of 2 · Gallery destination" : mobileStep === "plaque" ? "Step 2 of 2 · Curatorial plaque" : "Both choices ready"}
          </p>

          <div className="museum-floor">
            <article className="museum-artifact is-mobile-active">
            ...
            <section className={`museum-gallery-map ${wrongSelection === "gallery" || wrongSelection === "both" ? "is-wrong" : ""} ${mobileStep === "gallery" ? "is-mobile-active" : ""}`} aria-labelledby="gallery-choice-title">
            ...
            <section className={`museum-plaque-rack ${wrongSelection === "plaque" || wrongSelection === "both" ? "is-wrong" : ""} ${mobileStep === "plaque" ? "is-mobile-active" : ""}`} aria-labelledby="plaque-choice-title">
            ...
            {mobileStep === "ready" && selectedGallery && selectedPlaque && (
              <div className="museum-ready-recap is-mobile-active">
                <p>Both choices are ready. Change one, or install the exhibit below.</p>
                <button type="button" onClick={() => setSelectedGallery(null)}><small>Gallery</small><strong>{museumGalleriesById[selectedGallery].shortTitle}</strong><i>Change</i></button>
                <button type="button" onClick={() => setSelectedPlaque(null)}><small>Plaque</small><strong>{selectedPlaque}</strong><i>Change</i></button>
              </div>
            )}
          </div>
